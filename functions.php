...
function foundation_assets() {
	...
	if (!is_admin()) {
		...
		wp_enqueue_script( 'marc', get_template_directory_uri().'/js/marc.js');
...
function foundation_page_menu() {
	...
	'menu_class'  => 'large-12 columns center-nav',
	'include'     => '',
	'exclude'     => '',
	'echo'        => true,
	'show_home'   => false,
...
function register_index_menus() {
  register_nav_menus(
    array(
      'index-menu-1' => __( 'Profil Menu' ),
      'index-menu-2' => __( 'Vores Orgler Menu' ),
      'index-menu-3' => __( 'Aktuelt Menu' ),
      'index-menu-4' => __( 'Orgelbygning Menu' ),
      'marcussen-reuter' => __( 'Marcussen & Reuter' ),
      'orgelbevaegelsen' => __( 'Orgelbevægelsen' )
    )
  );
}
add_action( 'init', 'register_index_menus' );
...
function orgelcat_init() {
	register_taxonomy(
		'orgel-gruppe',
		'orgler',
		array(
			'label' => __( 'Orgelkategorier' ),
			'hierarchical' => true,
		)
	);
}
add_action( 'init', 'orgelcat_init' );

add_action( 'init', 'create_post_type' );
function create_post_type() {
	register_post_type('orgler', array(
        'labels' => array(
            'name' => 'Orgler',
            'singular_name' => 'Orgel',
            'add_new' => 'Nyt orgel',
            'edit_item' => 'Redigér orgel',
            'new_item' => 'Nyt orgel',
            'view_item' => 'Vis orgel',
            'search_items' => 'Søg efter orgel',
            'not_found' => 'Ingen orgler fundet',
            'not_found_in_trash' => 'Ingen orgler fundet i papirkurven'
        ),
        'public' => true,
        'hierarchical' => true,
    ));
